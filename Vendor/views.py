...
from .serializers import VendorProfileSerializers,PurchaseOrderSerializer,VendorPerformaceSerializer,HistoricalPerformanceSerializer
...
class Vendor_Historical_Performance(APIView):
    def get(self, request, pk, format=None):
        vendor = get_object_or_404(Vendor_Model, pk=pk)
        data = HistoricalPerformance.objects.filter(vendor=vendor)

        serializer=VendorPerformaceSerializer(data,many=True)
